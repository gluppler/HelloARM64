.text
//  Algorithms: Least Common Multiple (LCM)
//  Demonstrates LCM calculation with timing and detailed output
...
_start:
    ...
    //  Test 2: Median numbers (range 20-50, randomized)
    mov     x0, #STDOUT_FILENO
    adr     x1, test2_msg
    mov     x2, #test2_len
    mov     x8, #SYS_WRITE
    ...
    //  Test 3: Largest numbers (range 100-200, randomized)
    mov     x0, #STDOUT_FILENO
    adr     x1, test3_msg
    mov     x2, #test3_len
    mov     x8, #SYS_WRITE
...
no_borrow_lcm:
    ...
    //  Sanity check: elapsed time should be reasonable (< 1 second for these operations)
    cmp     x28, #1
    b.ge    time_error_lcm
    b       time_ok_lcm
...
print_microseconds:
    ...
    cmp     x22, #1
    b.eq    pad_2_digits_usec_lcm
    cmp     x22, #2
    b.eq    pad_1_digit_usec_lcm
    b       print_decimal_usec_lcm
    
pad_2_digits_usec_lcm:
    mov     x0, #STDOUT_FILENO
    adr     x1, zero_str
    mov     x2, #2
    mov     x8, #SYS_WRITE
...
print_nanoseconds:
    ...
    //  Pad to 3 digits if needed
    cmp     x22, #1
    b.eq    pad_2_digits_lcm
    cmp     x22, #2
    b.eq    pad_1_digit_lcm
    b       print_decimal_lcm
...
test2_msg:
    .asciz  "Test 2: Median Numbers (Randomized, range 20-50)\n"
test2_len = . - test2_msg

test3_msg:
    .asciz  "Test 3: Largest Numbers (Randomized, range 100-200)\n"
test3_len = . - test3_msg
