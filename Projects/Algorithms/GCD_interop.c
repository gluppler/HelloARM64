/*
 * GCD Interop - C wrapper for ARM64 GCD assembly
 */
...
extern uint64_t gcd_asm(uint64_t a, uint64_t b);
...
int main() {
    ...
    for (int i = 0; i < num_tests; i++) {
        ...
        uint64_t result = gcd_asm(a, b);
        
        clock_gettime(CLOCK_MONOTONIC, &end);
        
        printf("Result: [%llu]\n", result);
        printf("Status: GCD calculation completed successfully\n");
