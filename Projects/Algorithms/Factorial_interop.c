/*
 * Factorial Interop - C wrapper for ARM64 Factorial assembly
 */
...
extern uint64_t factorial_asm(uint64_t n);
...
int main() {
    ...
    uint64_t test_cases[] = {5, 10, 15, 20};
    ...
    for (int i = 0; i < num_tests; i++) {
        ...
        uint64_t result = factorial_asm(n);
        
        clock_gettime(CLOCK_MONOTONIC, &end);
        
        printf("Result: [%llu]\n", result);
        printf("Status: Factorial calculation completed successfully\n");
