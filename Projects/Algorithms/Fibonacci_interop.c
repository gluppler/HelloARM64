/*
 * Fibonacci Interop - C wrapper for ARM64 Fibonacci assembly
 */
...
extern uint64_t fibonacci_asm(uint64_t n);
...
int main() {
    ...
    uint64_t test_cases[] = {5, 20, 40, 50};
    ...
    for (int i = 0; i < num_tests; i++) {
        ...
        uint64_t result = fibonacci_asm(n);
        
        clock_gettime(CLOCK_MONOTONIC, &end);
        
        printf("Result: [%llu]\n", result);
        printf("Status: Fibonacci calculation completed successfully\n");
