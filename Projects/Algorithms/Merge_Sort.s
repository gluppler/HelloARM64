...
//  Demonstrates merge sort algorithm with timing and detailed output
...
copy_done:
    //  Save temp array pointer (x23) on stack before using it for time
    stp     x23, xzr, [sp, #-16]!
    ...
    //  Restore temp array pointer
    ldp     x0, xzr, [sp], #16
    mov     x3, x0                       //  temp array pointer
...
merge_sort:
    ...
    cmp     x20, #0
    b.lt    merge_sort_done
    cmp     x21, #1000
    b.gt    merge_sort_done
...
merge_loop:
    cmp     x24, x21
    b.gt    merge_left_done
    cmp     x25, x22
    b.gt    merge_right_done
    
    ldr     w0, [x19, x24, lsl #2]
    ldr     w1, [x19, x25, lsl #2]
    cmp     w0, w1
    b.gt    merge_right
