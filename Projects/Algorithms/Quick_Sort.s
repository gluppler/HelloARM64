...
//  Demonstrates quicksort algorithm with timing and detailed output
...
print_original_done:
    ...
    //  Allocate working array for sorting (same size as original array)
    mov     x0, x21
    lsl     x0, x0, #2
    add     x0, x0, #15
...
quicksort:
    ...
    //  Left recursion: quicksort(arr, low, pi - 1)
    mov     x0, x19
    mov     x1, x20
    sub     x2, x22, #1
    cmp     x2, x1
...
skip_left:
    //  Right recursion: quicksort(arr, pi + 1, high)
    add     x23, x22, #1
    cmp     x23, x21
    b.ge    quicksort_done
    mov     x0, x19
...
partition:
    ...
    //  Validate
    cmp     x20, x21
    b.ge    partition_error
    cmp     x20, #0
    b.lt    partition_error
    cmp     x21, #1000
    b.gt    partition_error
    
    ldr     w22, [x19, x21, lsl #2]        //  pivot = arr[high]
    sub     x23, x20, #1                    //  i = low - 1
    mov     x24, x20                        //  j = low
    
partition_loop:
    cmp     x24, x21                        //  j < high (not <=)
    b.ge    partition_done
    
    ldr     w25, [x19, x24, lsl #2]        //  arr[j]
    cmp     w25, w22                        //  Compare with pivot
    b.gt    no_increment
    
    add     x23, x23, #1                    //  i++
    //  Swap arr[i] and arr[j]
    ldr     w26, [x19, x23, lsl #2]        //  temp = arr[i]
    str     w25, [x19, x23, lsl #2]        //  arr[i] = arr[j]
    str     w26, [x19, x24, lsl #2]        //  arr[j] = temp
...
partition_done:
    ...
    ldr     w25, [x19, x23, lsl #2]        //  temp = arr[i+1]
    str     w22, [x19, x23, lsl #2]        //  arr[i+1] = pivot
    str     w25, [x19, x21, lsl #2]        //  arr[high] = temp
...
print_microseconds:
    ...
    cmp     x22, #1
    b.eq    pad_2_digits_usec_qs
    cmp     x22, #2
    b.eq    pad_1_digit_usec_qs
    b       print_decimal_usec_qs
    
pad_2_digits_usec_qs:
    mov     x0, #STDOUT_FILENO
    adr     x1, zero_str
    mov     x2, #2
    mov     x8, #SYS_WRITE
...
print_nanoseconds:
    ...
    //  Pad to 3 digits if needed
    cmp     x22, #1
    b.eq    pad_2_digits_nsec_qs
    cmp     x22, #2
    b.eq    pad_1_digit_nsec_qs
    b       print_decimal_nsec_qs
    
pad_2_digits_nsec_qs:
    mov     x0, #STDOUT_FILENO
    adr     x1, zero_str
    mov     x2, #2
    mov     x8, #SYS_WRITE
