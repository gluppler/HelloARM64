.text
//  Binutils: strip - Discard symbols from object files
//  Demonstrates symbol removal from ELF files in ARM64 assembly
...
_start:
    ...
    //  Open file for reading (use saved argv[1])
    ...
    //  Buffer already allocated, use x21
    ...
    //  Open file for writing (in-place modification)
    //  Use saved argv[1] from x20
    //  Use openat with AT_FDCWD
    mov     x0, #-100                   //  AT_FDCWD
    mov     x1, x20                     //  pathname (saved earlier)
    mov     x2, #O_WRONLY | O_TRUNC     //  flags
    mov     x3, #0                      //  mode
    mov     x8, #56                     //  SYS_openat
    svc     #0
    ...
    //  Write modified header (simplified - just write back)
    mov     x0, x19
    mov     x1, x20
    mov     x2, #64
    mov     x8, #SYS_WRITE
