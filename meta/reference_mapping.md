# Reference Mapping

This document maps all curriculum lessons and projects to their corresponding reference documentation in the authoritative [`../references/`](../references/) directory.

## Curriculum to References

### Lesson 00: Environment Setup
- references/tooling/
- references/external_links.md

### Lesson 01: Registers and Data
- references/registers_and_flags.md
- references/arm_architecture.md

### Lesson 02: Instruction Basics
- references/instruction_set.md
- references/arm_architecture.md

### Lesson 03: Control Flow
- references/instruction_set.md

### Lesson 04: Memory and Stack
- references/memory_management.md
- references/calling_conventions.md

### Lesson 05: Functions and ABI
- references/calling_conventions.md
- references/registers_and_flags.md

### Lesson 06: System Calls
- references/syscalls.md

### Lesson 07: Debugging
- references/debugging.md

### Lesson 08: Performance
- references/performance.md

### Lesson 09: Virtual Memory
- references/mmu_and_translation.md
- references/memory_management.md

## Projects to References

### Project 01: Userspace Tool
- references/syscalls.md
- references/calling_conventions.md

### Project 02: C/Assembly Runtime
- references/calling_conventions.md
- references/instruction_set.md

### Project 03: Bare Metal Firmware
- references/mmu_and_translation.md
- references/arm_architecture.md

### Project 04: Performance Lab
- references/performance.md
- references/instruction_set.md

## External References

All concepts traceable to:
- ARMv8-A Architecture Reference Manual
- ARM A64 Instruction Set Architecture
- AAPCS64 Calling Convention
- Official ARM documentation
